function [best_fitness, best_action, best_exe_cost, obj1, obj2, device_Energy, eh_real] = REMOVE(state, NUM_DEVICE, init_action)
    % GA parameters
%     NUM_DEVICE = 30;
    ...
    [~, ~, ~, ~, ~, ~, current_fitness, current_action, ~] ...
                = fitness_function(init_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
    
    flag = 1;
    while flag==1
        [flag_2to01,new_fitness,new_action]=exist_2to01(current_fitness, current_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
        if flag_2to01
            current_action = new_action;
            current_fitness = new_fitness;
        else
            [flag_01to2,new_fitness,new_action]=exist_01to2(current_fitness, current_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
            if flag_01to2
                current_action = new_action;
                current_fitness = new_fitness;
            else
                [flag_0to1,new_fitness,new_action]=exist_0to1(current_fitness, current_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
                if flag_0to1
                    current_action = new_action;
                    current_fitness = new_fitness;
                else
                    [flag_1to0,new_fitness,new_action]=exist_1to0(current_fitness, current_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
                    if flag_1to0
                        current_action = new_action;
                        current_fitness = new_fitness;
                    end
        ...
        if flag_2to01==0 && flag_01to2==0 && flag_0to1==0 && flag_1to0==0
            flag=0;
        end
    end


    [obj1, obj2, ~, ~, ~, device_Energy, best_fitness, best_action, ~]...
        = fitness_function(current_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);

    best_exe_cost = obj2/V;
...
% 卸载决策中遍历1、2，检查改为0是否更好
function [flag,new_fitness,last_action]=exist_2to01(current_fitness, action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter)
    ...
    for i=1:NUM_DEVICE
        if action(i)==2
            new_action0 = action;
            new_action0(i) = 0;
            [~, ~, ~, ~, ~, ~, new_fitness0, tmp_action0, ~] ...
                = fitness_function(new_action0, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
            new_action1 = action;
            new_action1(i) = 1;
            [~, ~, ~, ~, ~, ~, new_fitness1, tmp_action1, ~] ...
                = fitness_function(new_action1, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
            idx = min([new_fitness0,new_fitness1,current_fitness]);
            if idx==1
                flag = 1;
                last_action = tmp_action0;
                new_fitness = new_fitness0;
                break;
            elseif idx==2
                flag = 1;
                last_action = tmp_action1;
                new_fitness = new_fitness1;
                break;
            end
        end
    end    
...
% 卸载决策中遍历0，检查改为1、2是否更好
function [flag,new_fitness,last_action]=exist_01to2(current_fitness, action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter)
    new_fitness = current_fitness;
    last_action = action;
    flag = 0;
    for i=1:NUM_DEVICE
        if action(i)==0||action(i)==1
            new_action = action;
            new_action(i) = 2;
            [~, ~, ~, ~, ~, ~, new_fitness, tmp_action, ~] ...
                = fitness_function(new_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
            if new_fitness<current_fitness
                flag = 1;
                last_action = tmp_action;
                break;
            end
        end
    end
...
% 卸载决策中遍历1，检查改为2是否更好
function [flag,new_fitness,last_action]=exist_0to1(current_fitness, action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter)
    new_fitness = current_fitness;
    last_action = action;
    flag = 0;
    for i=1:NUM_DEVICE
...
% 卸载决策中遍历2，检查改为1是否更好
function [flag,new_fitness,last_action]=exist_1to0(current_fitness, action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter)
    new_fitness = current_fitness;
    last_action = action;
    flag = 0;
    for i=1:NUM_DEVICE
...
function [obj1, obj2, total_Delay, total_Energy, device_Delay, device_Energy, ftness, new_individual,count_num] = fitness_function(individual, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter)
    tau = parameter.tau;
    k = parameter.k;
