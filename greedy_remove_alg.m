...
function [results] = greedy_remove_alg(input_para)
...
%     disp(last_action);
    off_obj = off_obj_compu(last_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, Bat_level_virt, eh_real, parameter, bat_level, tau, E_max);
%     disp(off_obj);
%     if slot>500
%         disp("off_obj");
%         disp(off_obj);
...
%     disp(population);
    ...
    [~, best_action, ~, ~, ~, ~, ~] = REMOVE(state, NUM_DEVICE, best_action);
