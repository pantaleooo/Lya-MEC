function [best_fitness, best_action, best_exe_cost, obj1, obj2, device_Energy, eh_real] = markov_appro(state, NUM_DEVICE, init_action)
    % GA parameters
...
%     NUM_DEVICE = 30;
    ...
    for generation = 1:MAX_GENERATIONS
        randomIndex = randi(numel(best_action));
        new_action = best_action;
        new_action(randomIndex) = randi(3) - 1;

        [~, ~, ~, ~, ~, ~, new_fitness, new_action, ~] ...
...
%         if rand() <= 1/(1+exp((best_fitness-new_fitness)/best_fitness*100))
...
%         end
    ...
    [obj1, obj2, ~, ~, ~, device_Energy, best_fitness, ~, ~] ...
