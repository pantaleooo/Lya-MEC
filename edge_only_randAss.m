function [results] = edge_only_randAss(input_para)
    ...
    final_acton = ones(1, NUM_DEVICE);

    results = struct();

    [results.obj1, results.obj2, ~, ~, ~, results.device_Energy, results.best_obj, results.best_action, ~, results.reliab_task] ...
                = fitness_function(final_acton, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
