...
function [results] = greedy_markov(input_para)

%     NUM_DEVICE = 30;

    state = input_para.state;
    last_action = input_para.action;
    ...
    loc_obj = loc_obj_compu(Bat_level_virt, bat_level, E_max, eh_real, compu_task_bit, NUM_DEVICE, input_V, L, k, f_loc, tau);

    drop_obj = Bat_level_virt.*eh_real + input_V*alpha;
    
    loc_drop_min_obj = min(loc_obj, drop_obj);

    source_array = zeros(size(loc_drop_min_obj));  % 创建用于记录来源的数组，初始化为0
    source_array(drop_obj < loc_obj) = 2;  % 使用2标记来自drop_obj的元素
    source_array(loc_obj <= drop_obj) = 0;  % 使用0标记来自loc_obj的元素


    off_obj = off_obj_compu(last_action, NUM_DEVICE, bandwidth_up, bandwidth_down, ...
        compu_task_bit, download_task_bit, channel_gain, Bat_level_virt, eh_real, parameter, bat_level, tau, E_max);


    not_off = off_obj==99999;


    decision_gain = loc_drop_min_obj - off_obj;

    

    decision_gain(not_off) = -99999;


    [~, sorted_index] = sort(decision_gain, 'descend');


    population = zeros(NUM_DEVICE);

    for i=1:NUM_DEVICE
        index = sorted_index(1:i);
        population(i,index) = 1;
        zero_indices = (population(i,:) == 0);
        population(i,zero_indices) = source_array(zero_indices);
        population(i,not_off) = source_array(not_off);
    end
%     disp("population")
%     disp(population);

    index = find(decision_gain<0);
    for i=1:NUM_DEVICE
        population(i,index) = source_array(index);
    end

    best_action = zeros(1,NUM_DEVICE);
    best_obj = 99999;



    for i=1:NUM_DEVICE
        [~, ~, ~, ~, ~, ~, obj, solution, ~,~] = fitness_function(population(i,:), NUM_DEVICE, bandwidth_up, ...
            bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
        if obj<best_obj
            best_obj = obj;
            best_action = solution;
        end
    end

    [~, best_action, ~, ~, ~, ~, ~] = markov_appro(state, NUM_DEVICE, best_action);
    ...
    [results.obj1, results.obj2, ~, ~, ~, results.device_Energy, results.best_obj, results.best_action, ~,results.reliab_task] ...
                = fitness_function(best_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, bat_level, Bat_level_virt, eh_real, parameter);
    results.best_exec_cost = results.obj2/input_V;
    results.eh_real = eh_real;
%     disp(results.best_action);
...
function loc_obj = loc_obj_compu(Bat_level_virt, bat_level, E_max, eh_real, theta, N, V, L, K, f_loc, tau)
...
function off_obj = off_obj_compu(last_action, NUM_DEVICE, bandwidth_up, bandwidth_down, compu_task_bit, download_task_bit, channel_gain, Bat_level_virt, eh_real, parameter, bat_level, tau, E_max)
    ...
    off_obj = zeros(1,NUM_DEVICE);

    for i=1:NUM_DEVICE

        individual = last_action;
        individual(i) = 1;
%         disp(individual);
...
%         precomputed_values = sqrt(L .* compu_task_bit);
% 
%         index = individual~=1;
% 
%         precomputed_values(index) = 0;
%     
%         tmp = sum(precomputed_values);
%     
%         total_fre = sum(F);
%     
%         f_off = precomputed_values ./ tmp * total_fre;
%         device_Delay = compu_task_bit(i) / r_up(i) + compu_task_bit(i) * L(i) / sum(f_off(:,i)) + download_task_bit(i) / r_down(i); % Upload + Compute + Download delay
%     
%         device_Energy = p_up(i) * compu_task_bit(i) / r_up(i) + p_down(i) * download_task_bit(i) / r_down(i); % Upload + Download energy
% 
%         if device_Delay <= tau && device_Energy <= bat_level(i) && device_Energy <= E_max(i)
%             off_obj(i) = Bat_level_virt(i) * (eh_real(i) - device_Energy) + V * device_Delay;
%         else
%             off_obj(i) = 99999;
%         end
